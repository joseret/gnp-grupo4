    ...
    'success': function(json_data) {
      ...
      if ( json_data['list'] && json_data['list'].length > 0 ) {
        ...
        for(var o in json_data['list']) {
          ...
          $("#messages").append(innerInfo);
          var innerInfo = $('<div  style="padding-bottom: 2px"  />');
          innerInfo.append( $('<span style="padding-right: 10px" />').text('Telefono'));
          innerInfo.append( $('<span style="padding-right: 10px" />').text(json_data['list'][o]['telefono']));
          $("#messages").append(innerInfo);
          var innerInfo = $('<div  style="padding-bottom: 10px"  />');
          innerInfo.append( $('<span style="padding-right: 10px" />').text('Su póliza GNP Individual se registró con fecha:D'));
...
FriendlyChat.prototype.submitButtonAction = function() {
    ...
    var self = this;
    var info = {'policy_id': $('#reference').val(), 'nickname': $('#nickname').val(),'telefono': $('#telefono').val()};
